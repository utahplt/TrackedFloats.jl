...
@testset "maxFrames: only print out n stack frames" begin
  ...
  tf_config_logger(filename=tmp1, buffersize=1)
  f0(floaty)
  tf_flush_logs()
  ...
  tf_config_logger(filename=tmp2, maxFrames=3)
  f0(floaty)
  tf_flush_logs()
...
@testset "maxLogs: only log n events then stop" begin
  ...
  tf_config_logger(filename=tmp1, maxFrames=1, buffersize=1)
  f0(floaty)
  tf_flush_logs()
  ...
  tf_config_logger(filename=tmp2, maxLogs=3)
  f0(floaty)
  tf_flush_logs()
